...
iwm_alloc_rx_ring(struct iwm_softc *sc, struct iwm_rx_ring *ring)
{
    ...
    size = count * descsz;
    err = iwm_dma_contig_alloc(sc->sc_dmat, &ring->free_desc_dma, (void**)&ring->desc, size, 256);
    if (err) {
        XYLog("%s: could not allocate RX ring DMA memory\n",
              DEVNAME(sc));
        goto fail;
    }
    ring->desc = (uint32_t*)ring->free_desc_dma.vaddr;
    ...
    if (sc->cfg->mqrx_supported) {
        ...
        err = iwm_dma_contig_alloc(sc->sc_dmat, &ring->used_desc_dma, NULL,
                                   size, 256);
        if (err != 0) {
            XYLog("could not allocate RX ring DMA memory\n");
            goto fail;
        }
    }
...
iwm_rx_addbuf(struct iwm_softc *sc, int size, int idx)
{
    ...
    if (sc->cfg->mqrx_supported)
        ((uint64_t *)ring->desc)[idx] = htole64(data->map->dm_segs[0].location);
    else
