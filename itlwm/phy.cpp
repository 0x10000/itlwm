                            ...
                            uint16_t ch_id)
{
    ...
    if (type == IWM_PHY_DB_CALIB_CHG_PAPD)
        ...
        ch_group_id = iwm_channel_id_to_txp(phy_db, ch_id);
    
    entry = iwm_phy_db_get_section(phy_db, (enum iwm_phy_db_section_type)type, ch_group_id);
    if (!entry)
        return EINVAL;
    ...
    *size = entry->size;
    ...
    XYLog("%s(%d): [PHYDB] GET: Type %d , Size: %d\n",
          __func__, __LINE__, type, *size);
                    ...
                    uint16_t length, void *data)
{
    ...
    XYLog("Sending PHY-DB hcmd of type %d, of length %d\n",
                type, length);
                                   ...
                                   uint8_t max_ch_groups)
{
    ...
    for (i = 0; i < max_ch_groups; i++) {
        ...
        err = iwm_send_phy_db_cmd(phy_db,
                                  ...
                                  entry->data);
        if (err) {
            XYLog("Can't SEND phy_db section %d (%d), err %d\n",
                          type, i, err);
            return err;
        }
        
        XYLog("Sent PHY_DB HCMD, type = %d num = %d\n", type, i);
    }
...
iwm_send_phy_db_data(struct iwm_phy_db *phy_db)
{
    ...
    XYLog("%s: Sending phy db data and configuration to runtime image\n",
          __func__);
    ...
    XYLog("%s: Finished sending phy db non channel data\n",
                __func__);
    return 0;
}
                        ...
                        const struct iwm_time_event_cmd_v2 *cmd)
{
    ...
    if (sc->sc_fw.ucode_capa.flags & IWM_UCODE_TLV_FLAGS_TIME_EVENT_API_V2) {
        hcmd.data[0] = cmd;
...
iwm_send_phy_cfg_cmd(struct iwm_softc *sc)
{
    ...
    phy_cfg_cmd.phy_cfg = htole32(sc->sc_fw.phy_config);
                    ...
                    uint32_t *status)
{
    ...
    _KASSERT((cmd->flags & IWM_CMD_WANT_RESP) == 0);
